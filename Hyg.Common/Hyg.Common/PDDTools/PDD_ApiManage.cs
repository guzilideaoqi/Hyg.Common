            ...
            try
            {
                if (WaitConvertGoodInfoList is List<Good_Search_Item>)
                {
                    ...
                    foreach (Good_Search_Item item in good_Search_ItemList)
                    {
                        ...
                        {
                            skuid = item.goods_sign.ToString(),
                            title = item.goods_name,
                            shopId = item.mall_id.ToString(),
                            shopName = item.mall_name,
                            coupon_after_price = Math.Round((item.min_group_price - item.coupon_discount), 2).ToString(),
                            coupon_price = Math.Round(item.coupon_discount, 2).ToString(),
                            origin_price = Math.Round(item.min_group_price, 2).ToString(),
                            ...
                            TotalCommission = Math.Round((double)((item.min_group_price - item.coupon_discount) * item.promotion_rate) / 100, 2),
                            PlaformType = 4,
